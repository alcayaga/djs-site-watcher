...
1.  **Branch Management**: **CRITICAL:** Ensure you are NOT working on the
    ...
    - If the current branch is `master`, you MUST create and switch to a new
      descriptive branch:
      ```bash
...
8.  **Create PR**: Use the `gh` CLI to create the PR. To avoid shell escaping
    issues with multi-line Markdown, you **MUST** escape backticks
    ```bash
    gh pr create --title "type(scope): succinct description" --body "The legacy global check in \`interactionHandler.js\`"
    ```
