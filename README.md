...
3. Configure the bot in `config/settings.json`:
   ...
   - `uptimeKumaUrl` (Optional): A Push URL from [Uptime Kuma](https://github.com/louislam/uptime-kuma) to ping as a heartbeat on the same interval. Can also be set in `.env` as `UPTIME_KUMA_URL`.
